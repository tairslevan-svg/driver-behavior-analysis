...
object ThreeConsecutiveOverspeedAlertToConsole {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val checkpointDir = if (args.length > 3) args(3) else "hdfs:///tmp/week7/three_consecutive_overspeed_alert_ckpt"
    val overspeedThreshold = if (args.length > 4) args(4).toDouble else 120.0
    val alertCount = if (args.length > 5) args(5).toInt else 3

    val conf = new SparkConf()
      .setAppName("ThreeConsecutiveOverspeedAlertToConsole")
    ...
    val updateFunction = (vehicleId: String, speedOption: Option[Double], state: State[Int]) => {
      ...
      val alertFlag = if (newCount >= alertCount) "TRIGGERED" else "WAITING"
      s"vehicle_id=$vehicleId\tspeed=$speed\tconsecutive_overspeed_count=$newCount\talert=$alertFlag"
    }
    ...
    updates.foreachRDD { rdd =>
      if (rdd.isEmpty()) {
        println("[ThreeConsecutiveOverspeedAlertToConsole] no data in this batch")
      } else {
        println("[ThreeConsecutiveOverspeedAlertToConsole] batch result:")
